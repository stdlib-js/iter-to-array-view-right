...
function iterator2arrayviewRight( iterator, out ) {
	...
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '0Pz46', iterator ) );
	}
	if ( !isCollection( out ) ) {
		throw new TypeError( format( '0Pz2y', out ) );
	}
	...
	if ( nargs === 2 ) {
		...
		if ( isFunction( arguments[ 3 ] ) ) {
			...
			fcn = arguments[ 4 ];
			if ( !isFunction( fcn ) ) {
				throw new TypeError( format( '0Pz31', fcn ) );
			}
		}
	} else { // nargs > 5
		begin = arguments[ 2 ];
	...
	if ( !isInteger( begin ) ) {
		throw new TypeError( format( '0PzE2', begin ) );
	}
	if ( !isInteger( end ) ) {
		throw new TypeError( format( '0PzE3', end ) );
	}
	if ( end < 0 ) {
		end = out.length + end;
		if ( end < 0 ) {
