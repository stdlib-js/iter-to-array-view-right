...
function iterator2arrayviewRight( iterator, out ) {
	...
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '0Pz46,G5', iterator ) );
	}
	if ( !isCollection( out ) ) {
		throw new TypeError( format( '0Pz2y,Ix', out ) );
	}
	...
	if ( nargs === 2 ) {
		...
		if ( isFunction( arguments[ 3 ] ) ) {
			...
			fcn = arguments[ 4 ];
			if ( !isFunction( fcn ) ) {
				throw new TypeError( format( '0Pz31,Lk', fcn ) );
			}
		}
	} else { // nargs > 5
		begin = arguments[ 2 ];
	...
	if ( !isInteger( begin ) ) {
		throw new TypeError( format( '0PzLl', begin ) );
	}
	if ( !isInteger( end ) ) {
		throw new TypeError( format( '0PzLm', end ) );
	}
	if ( end < 0 ) {
		end = out.length + end;
		if ( end < 0 ) {
